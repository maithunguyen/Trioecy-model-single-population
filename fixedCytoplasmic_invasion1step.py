...
for pollen_threshold in np.around([1], decimals=4):
    for csm in np.around([0.8], decimals=4):
        for alpha in np.around(np.linspace(0.5, 20, 50), decimals=4):  # np.linspace(0.5,60,200)
            for s in np.around([0.4], decimals=4):  # np.linspace(0,0.95,40) 0.3
                for d in np.around([0.1], decimals=4):  # np.linspace(0,0.95,40) 0.1
                    for g in np.around(np.linspace(0.7, 10, 50), decimals=4):  # np.linspace(0.4,1.7,40)
                        Aa = 0.000001
                        Aac = 0
                        AAc = 0.8
                        AA = 1 - Aa - AAc
...
lgyn = s*(1-d)/(g+s-1)
lgyn = (s*(x*(1-csm)-2)*d/(x*(1-csm)-2)-s)/(1-s)
g = np.linspace(1.2,10,100)
a = (0.1728 + 1.1232 * g - 3.12 * g * g + 2 * g * g*g)/(0.16416 + 0.432 * g - 1.56 * g * g + g * g*g)
plt.plot(a, g, lw = 3, c = "grey")

x = np.linspace(0.5,20,100)

plt.plot(x, poly5, lw = 3, c = "grey")
plt.plot(x, (1-d)/l +s -1, lw = 3, c = "black")
plt.plot([2*(1-s*d)/(1-s)]*100, np.linspace(0.7,10,100), lw = 3, c = "pink")
